---
layout: event
title: "Посетители в Красном"
slug: visitors_dyed_in_red
chapters_count: 18
black_scene:
  - "В бескрайнем цикле перерождений даже время утратило свой смысл."
  - "Приближение от неизбежности к вероятности..."
  - "Наследие, переходящее от бесплотного к осязаемому."
  - "Если бы тогда не был сделан тот самый выбор, Мог бы у этой истории быть иной финал?"
  - "И какой пейзаж отражался в Ваших глазах тогда..."
  - "О, Великий Создатель?"
---

  <div class="chapter" id="part1"> <!-- начало главы -->
  <p class="title-1">Приказ</p>
  {% include blackscreen.html lines=page.black_scene %}
  {% assign rows = site.data.visitors_dyed_in_red.part1.chapter1.rows %}
  {% include dialog.html rows=rows bg_class="table-bg" %}
  {% include flash.html type="into-white" %}
  {% include flash.html type="from-white" %}
  {% assign rows = site.data.visitors_dyed_in_red.part1.chapter2.rows %}
  {% include dialog.html rows=rows bg_class="table-bg" %}
  </div> <!-- конец главы -->
  ...
  {% assign rows = site.data.visitors_dyed_in_red.part4.chapter1.rows %}
  {% include dialog.html rows=rows bg_class="table-bg" %}
  </div> <!-- конец главы -->
  ...
  <p class="title-1">Око бури 2</p>
  {% assign rows = site.data.visitors_dyed_in_red.part10.chapter1.rows %}
  {% include dialog.html rows=rows bg_class="table-bg" %}
  </div> <!-- конец главы -->
  ...
  <p class="title-1">Связь с Сестрой - Часть 1</p>
  {% include divider.html %}
